...
@section('content')
    ...
    <div class="flex flex-col items-center justify-center px-6 pt-8 mx-auto md:h-screen pt:mt-0">
        <div class="w-full p-10 bg-white rounded-lg shadow md:mt-0 sm:max-w-md">
            <h1 class="text-2xl font-bold text-center text-gray-900">Registration</h1>
            <form class="mt-8 space-y-6" method="POST" action="{{ route('register') }}">
                ...
                <button type="submit"
                    class="w-full py-2 font-medium text-center text-white rounded-lg bg-cyan-600 hover:bg-cyan-700">Register</button>

                <div class="flex justify-between">
                    <a href="{{ route('login') }}"
                        class="text-sm font-medium hover:underline text-cyan-600">Login</a>
                    <a href="{{ route('dashboard') }}" class="text-sm font-medium text-gray-700 hover:underline">Home</a>
                </div>
            </form>
        </div>
    </div>
