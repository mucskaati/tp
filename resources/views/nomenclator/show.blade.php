            ...
            <div class="flex flex-col-reverse gap-10 lg:flex-row">
                ...
                <div id="nomenclator-gallery" class="w-full lg:max-w-xl">
                    @if (isset($key['images']) && count($key['images']) > 0)
                        <a href="{{ $key['images'][0]['url'] ?: asset('images/default-image-large.png') }}"
                            target="_blank">
                            <img src="{{ $key['images'][0]['url'] ?: asset('images/default-image-large.png') }}"
                                class="rounded @if ($key['images'][0]['hasInstructions']) border-[3px] border-blue-500 @endif"
                                alt="{{ $key['signature'] }}" />
                                ...
                                @if ($index != 0)
                                    <a href="{{ $image['url'] }}" target="_blank">
                                        ...
                                        <img src="{{ $image['url'] }}" alt="{{ $key['signature'] }}"
                                            class="object-cover w-20 h-20 rounded @if ($image['hasInstructions']) border-[3px] border-blue-500 @endif" />
                                        <span class="pswp-caption-content">Has instructions:
                    ...
                    @if (isUserSubmitter($key))
                        <div class="mb-5">
                            <div class="flex items-center justify-end gap-5">
                                {{-- Edit key and Edit state should be either in same row, or same size of button --}}
                                <a href="{{ route('nomenclator.edit', $key['id']) }}" class="btn btn-primary">
                                    Edit key
                                </a>
                            </div>
                        </div>
                    ...
                    @if (loggedIsAdmin())
                        <div class="mb-5">
                            <div class="flex items-center justify-between gap-5 ">
                                <span
                                    class="inline-block px-3 py-1 text-sm text-white {{ getStateColor($key['state']['state'])->bg }} rounded">
                                    State: {{ ucfirst($key['state']['state']) }}
                                </span>
                                <a href="{{ route('nomenclator.edit_state', $key['id']) }}" class="btn btn-primary">
                                    Edit state
                                </a>
                            </div>
                            @if ($key['state']['note'])
                                <div
                                    class="flex items-center gap-3 p-4 mt-4 bg-white border-b {{ getStateColor($key['state']['state'])->border }} rounded shadow-md">
                                    <x-heroicon-o-information-circle
                                        class="w-5 h-5 {{ getStateColor($key['state']['state'])->text }}" />
                    ...
                    <div class="flex gap-5 text-sm text-gray-500 justify-evenly md:gap-10">
                        ...
                        <ul class="flex flex-col gap-6">
                            ...
                            <li>
                                ...
                                <span
                                    class="text-base text-blue-400">{{ isset($key['placeOfCreation']['name']) ? $key['placeOfCreation']['name'] : 'Unknown' }}</span>
                            </li>
                            <li>
                                <div class="flex items-center gap-2 mb-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none"
