...
@section('content')
    ...
    <edit-key-users inline-template v-cloak :all-key-users="{{ $keyUsers->toJSON() }}" nomenclator-key-id="{{ $key['id'] }}" pre-submitted-key-users="{{ json_encode($key['keyUsers']) }}">
        <div class="container">
            <div class="mt-4 box">
                <h1 class="mb-5 text-xl font-semibold text-center text-gray-900">Edit users of a cipher key {{ $key['signature'] }}</h1>
            </div>
            <div class="grid grid-cols-4 gap-4 mt-3 mb-3" v-for="(keyUser, index) in keyUsers" :key="`keyUsers${index}`">
                <div class="form-element">
                    <label for="keyUserId" class="input-label">
                        Choose a key user <span class="input-label-error">
                            @error('keyUserId')
                                {{ $message }}
                            @enderror
                        </span>
                    </label>
                    <select name="keyUserId[]" id="keyUserId[]" v-model="keyUser.id" class="input @error('keyUserId') input-error @enderror" :disabled="keyUser.persisted">
                ...
                <div class="flex items-center form-element">
                    <label :for="'main' + index" class="input-label">
                        <input :name="'keyUserMain[' + index + ']'" {{-- value="1" :checked="keyUser.isMainUser" --}} v-model="keyUser.isMainUser" type="checkbox" :id="'main' + index" class="input @error('keyUserMain') input-error @enderror" :disabled="keyUser.persisted">
